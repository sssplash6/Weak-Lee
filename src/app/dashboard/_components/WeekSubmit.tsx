...
/**
 * The weekly goal-submission control. Before submitting, a prompt + "Submit
 * goals" button confirms the goal list (which then locks it for the week).
 * Once submitted, it shows when and offers "Edit goals" to re-open for changes.
 */
export function WeekSubmit({
  ...
  return (
    <div className="mb-5 flex items-center gap-3 rounded-xl border border-dashed border-line bg-surface/50 px-4 py-3">
      ...
      >
        {isPending ? "Submitting…" : "Submit goals"}
      </button>
    </div>
  );
}
